...
@Component
class ElectionsTransformationImpl extends ElectionsTransformation {
    ...
    @Autowired
    private var options: ElectionsOptions = _
    ...
    private def readCellValue(cell: Cell,
        ...
        Option(cell)
            .map(cell => StringUtils.trimToEmpty(formatter.formatCellValue(cell, evaluator)))
            .orNull
    }
